/*
 * ngx_http_heavybag_module - Apache HTTP fingerprint spoofing
 *
 * See heavybag_spoof.h for the design. Detection of an nginx built-in error
...
typedef struct {
    ...
    ngx_str_t    reason;
    ngx_str_t    detail;
} ngx_http_heavybag_errtext_t;


static ngx_http_heavybag_errtext_t  ngx_http_heavybag_errtexts[] = {
...
static ngx_str_t  ngx_http_heavybag_generic_reason = ngx_string("Error");
static ngx_str_t  ngx_http_heavybag_generic_detail =
    ngx_string("The server encountered an error processing your request.");


static ngx_int_t ngx_http_heavybag_spoof_header_filter(ngx_http_request_t *r);
static ngx_int_t ngx_http_heavybag_spoof_body_filter(ngx_http_request_t *r,
    ngx_chain_t *in);
static ngx_int_t ngx_http_heavybag_build_error_page(ngx_http_request_t *r,
    ngx_str_t *token, ngx_str_t *out);


static void
ngx_http_heavybag_errtext_lookup(ngx_uint_t status, ngx_str_t **reason,
    ngx_str_t **detail)
{
    ngx_uint_t  i;

    for (i = 0; ngx_http_heavybag_errtexts[i].status; i++) {
        if (ngx_http_heavybag_errtexts[i].status == status) {
            *reason = &ngx_http_heavybag_errtexts[i].reason;
            *detail = &ngx_http_heavybag_errtexts[i].detail;
            return;
        }
    }

    *reason = &ngx_http_heavybag_generic_reason;
    *detail = &ngx_http_heavybag_generic_detail;
}
 ...
 */
static ngx_int_t
ngx_http_heavybag_build_error_page(ngx_http_request_t *r, ngx_str_t *token,
    ngx_str_t *out)
{
    u_char      *p;
    size_t       size;
    ...
    ngx_http_heavybag_errtext_lookup(status, &reason, &detail);
 ...
 */
static ngx_int_t
ngx_http_heavybag_spoof_header_filter(ngx_http_request_t *r)
{
    ngx_table_elt_t          *h;
           ...
           == 0)
    {
        ...
        if (ngx_http_heavybag_build_error_page(r, &wlcf->server_token,
                                          &ctx->spoof_body)
            != NGX_OK)
        {
            return NGX_ERROR;
 ...
 */
static ngx_int_t
ngx_http_heavybag_spoof_body_filter(ngx_http_request_t *r, ngx_chain_t *in)
{
    ngx_buf_t           *b;
    ngx_chain_t          out;
    ngx_http_heavybag_ctx_t  *ctx;
...
ngx_int_t
ngx_http_heavybag_spoof_init(ngx_conf_t *cf)
{
    ngx_http_next_header_filter = ngx_http_top_header_filter;
    ngx_http_top_header_filter = ngx_http_heavybag_spoof_header_filter;

    ngx_http_next_body_filter = ngx_http_top_body_filter;
    ngx_http_top_body_filter = ngx_http_heavybag_spoof_body_filter;
