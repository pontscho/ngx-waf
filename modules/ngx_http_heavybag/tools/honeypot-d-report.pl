...
# heavybag loads and changes no module .c. The runtime levers it feeds
...
# The geo values come exclusively from `geolookup` (the bounds-guarded heavybag_geo.c
...
# --------------------------------------------------------------------------
# libloc network-flag bits (heavybag_geo.h:33-36). `tor` is the T1 country code,
