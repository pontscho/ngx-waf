/*
 * ngx_http_waf_module - JA4 TLS client fingerprint
 *
 * See waf_ja4.h. This file splits into two regions:
 *   - the PURE core (ngx_http_waf_ja4_build + helpers), nginx-free, compiled
 *     standalone under -DWAF_JA4_UNIT_TEST for the JA4 unit test;
 *   - the SSL extractor / handshake callback, wrapped in
 *     `#ifndef WAF_JA4_UNIT_TEST` so the core compiles without nginx/SSL.
 *
 * The pure core treats every input as attacker-controlled: GREASE-filtered
 * and clamped to WAF_JA4_MAX_ELEMS before any copy/sort (CWE-190/787), output
...
int
ngx_http_waf_ja4_build(const uint16_t *ciphers, size_t n_ciphers,
    ...
    uint16_t version, int is_quic, char out[WAF_JA4_LEN])
{
    ...
    char       ver[2];
    char       alpn_c[2];
    char       csv[WAF_JA4_MAX_ELEMS * 5 * 2 + 4];
    uint16_t   list[WAF_JA4_MAX_ELEMS];
    uint16_t   sig[WAF_JA4_MAX_ELEMS];
    size_t     i, n, ns, pos;
    int        sni, ncip, next;
    ...
    n = ja4_filter_grease(ciphers, n_ciphers, list, WAF_JA4_MAX_ELEMS);
    if (n > 1) {
    ...
    n = 0;
    for (i = 0; i < n_exts && n < WAF_JA4_MAX_ELEMS; i++) {
        if (ja4_is_grease(exts[i])
    ...
    ns = 0;
    if (sigalgs != NULL) {
        ns = ja4_filter_grease(sigalgs, n_sigalgs, sig, WAF_JA4_MAX_ELEMS);
    }
    ...
    (void) snprintf(out, WAF_JA4_LEN, "%s_%s_%s", a, jb, jc);
 ...
 *
 * ngx_http_waf_ja4_compute() (task-014) reads the ClientHello getters off an
 * SSL* and feeds ngx_http_waf_ja4_build(); the client_hello callback + SSL_CTX
 * wiring (task-015) live in ngx_http_waf_module.c. Implemented in later tasks;
 ...
 */
ngx_int_t
ngx_http_waf_ja4_compute(SSL *ssl, ngx_pool_t *pool, ngx_str_t *out)
{
    int                   *ext_ids = NULL;
    const unsigned char   *p;
    size_t                 n, i, len, end, list;
    size_t                 n_cip, n_ext, n_sig, alpn_len;
    uint16_t               ciphers[WAF_JA4_MAX_ELEMS];
    uint16_t               exts[WAF_JA4_MAX_ELEMS];
    uint16_t               sigalgs[WAF_JA4_MAX_ELEMS];
    uint16_t               version, hi, v;
    const uint8_t         *alpn;
    int                    is_quic;
    char                   buf[WAF_JA4_LEN];
    u_char                *dst;
    ...
    for (i = 0; i + 1 < len && n_cip < WAF_JA4_MAX_ELEMS; i += 2) {
    ...
    if (SSL_client_hello_get1_extensions_present(ssl, &ext_ids, &n) == 1
        && ext_ids != NULL)
    {
        for (i = 0; i < n && n_ext < WAF_JA4_MAX_ELEMS; i++) {
    ...
    if (SSL_client_hello_get0_ext(ssl, 0x000d, &p, &len) == 1 && len >= 2) {
        ...
        for (i = 2; i + 1 < end && n_sig < WAF_JA4_MAX_ELEMS; i += 2) {
    ...
    if (ngx_http_waf_ja4_build(ciphers, n_cip, exts, n_ext, sigalgs, n_sig,
